...
**Image Accept parity (AVIF/WebP)** — `nginx/sqrd-page-cache.conf` resolves `$sqrd_img_ext` from the client `Accept` header (`image/avif` → `.avif`, else `image/webp` → `.webp`, else empty) and serves uploaded JPEG/PNG variants via a scoped `location ~* ^/wp-content/uploads/...`. `Negotiation::image_ext_for_accept()` in PHP is the parity mirror (avif → webp → null) — NOT invoked on the image request path today (images bypass PHP); it exists as a testable mirror and for future PHP-side delivery layers. Both MUST change in lockstep if the priority or supported formats change. The plugin only SERVES variants — generation is delegated to a third-party conversion plugin. The `try_files` walks three sibling conventions: appended same-dir (`img.jpg.avif` — CompressX, WebP Express "Mingled", AVIF Express, ShortPixel/Imagify/EWWW), replaced same-dir (`img.avif`), and Converter for Media's separate `wp-content/uploads-webpc/` dir. When extending to support a new plugin, check where it writes variants and add a `try_files` candidate if it uses a new convention. Documented supported combos: CompressX (solo, both formats), WebP Express + AVIF Express (combo, one format each). Users MUST disable the conversion plugin's own delivery/rewrite and let nginx serve; the uploads-path assumption is the WP default `/wp-content/uploads/`. Scope is limited to `jpe?g|png` originals (animated-GIF→WebP/AVIF is plugin-specific and lossy; WebP/AVIF originals need no renegotiation).

**Brotli nginx config split** — brotli_static on and brotli_types live in nginx/brotli-static.conf (a SEPARATE optional include), NOT in nginx/sqrd-page-cache.conf. The main include only sets gzip_static on / gzip_vary on (stock nginx). Reason: brotli_static is an ngx_brotli directive — emitting it unconditionally would make nginx refuse to start on any build without the module. Users with ngx_brotli include both files; users without include only the main one. If you ever need to change brotli serving behaviour, edit nginx/brotli-static.conf. The admin Settings page shows an info notice pointing to the include when the PHP brotli extension is loaded, and a warning when it is not.

**Compressed-sibling skip-if-larger** — Store::write_body() skips writing a .gz / .br sibling when strlen($compressed) >= strlen($original). nginx gzip_static / brotli_static serve whatever sibling exists on disk to any client that sends Accept-Encoding; without this guard, a tiny body (short Markdown, error stubs) would get a *larger* compressed sibling served. Do NOT remove this check. The guard compares against the pre-minification body byte count (minification runs before write_body is called in Output::finish()), so the sibling is always compared against the exact bytes nginx will serve as the plain fallback.

**Compression quality filters** — Store::gzip_level() (default 6, clamped 0–9) and Store::brotli_quality() (default 5, clamped 0–11) are the filterable compression levels, hooked via sqrd_page_cache/gzip_level and sqrd_page_cache/brotli_quality. The defaults mirror each other's cost/ratio on typical HTML. Do not inline raw integers in write_body() — always go through the helpers so the filters and clamping apply.
