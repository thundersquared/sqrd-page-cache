...
* **Hardening — compressed siblings larger than the original are skipped.** Store::write_body() now compares each .gz / .br sibling against the original byte count and skips writing it when compression would inflate the file. Previously, tiny bodies (e.g. short Markdown) could produce a .br/.gz larger than the plain file, and nginx gzip_static/brotli_static would serve the bigger blob to any client that sent Accept-Encoding. Without the sibling on disk, nginx transparently falls back to the plain file.
* **Feature — tunable compression quality.** New filters sqrd_page_cache/gzip_level (default 6, range 0–9) and sqrd_page_cache/brotli_quality (default 5, range 0–11) expose the pre-compression levels that were previously hardcoded constants. Both are clamped to their valid ranges.
* **Diagnostics — compression failures are now logged.** gzencode / brotli_compress returning false (previously swallowed silently) now emits an error_log line so admins can diagnose missing .gz / .br siblings.
